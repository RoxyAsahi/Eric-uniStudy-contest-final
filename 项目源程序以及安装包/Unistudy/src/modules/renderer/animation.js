...
function processScripts(containerElement) {
    ...
    const classicScriptTypes = new Set([
        '',
        'text/javascript',
    ...
    const executableScripts = allScripts.filter(s => {
        const type = (s.getAttribute('type') || '').trim().toLowerCase();
        return classicScriptTypes.has(type);
    });
    ...
    const inlineScripts = executableScripts.filter(s => !s.src && s.textContent.trim());
